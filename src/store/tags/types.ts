...
export class TagStorage {
  ...
  private register (tag: string): TagInfo {
    ...
    if (existingInfo) return existingInfo
      ...
      const parsedValue = this.splitTag(value)
      const referenced = parsedValue.flatMap(pv => pv.type === 'reference' ? pv.required : [])
      const dependencies = [...isA, ...referenced].map(t => this.register(t))
  ...
  private splitTag (tag: string): ParsedTag[] {
    let res: ParsedTag[] = []
    const re = /#{([^}]*)}/g
    let index = 0
    let array
    ...
    if (init) {
      this.add(
        ...data.taxonomy.map(t => ({
          tag: t.tag,
          value: t.value,
          isA: t.extends ? [t.extends] : undefined,
          requires: t.requires ?? [],
          excludes: t.excludes ?? [],
          groups: []
        })),
